# ============================================================
# Week 19 - FLAPPY BIRD #3 : บันทึกสถิติลงไฟล์  (เฉลย)
# แนวคิด: เขียนไฟล์ด้วย with open(...), โหมด w และ a, บันทึกเท่าที่จำเป็น
...
while running:
    ...
    if state == "playing":
        ...
        # ---------- จบเกม: บันทึกข้อมูล (ทำครั้งเดียว) ----------
        if state == "gameover":
            rounds = rounds + 1
            save_history()
            if score > best:
    ...
    screen.blit(font_sml.render(f"เล่นไป {rounds} รอบ", True, DARK), (20, 50))
